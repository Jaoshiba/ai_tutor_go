...
func EmbeddingText(co *cohereClient.Client, text string, ctx *fiber.Ctx) ([]float32, error) {
	...
	req := openai.EmbeddingRequest{
		Input: []string{text}, // ต้องส่งเป็น slice ของ string
		Model: model,
	}

	resp, err := co.CreateEmbeddings(ctx.Context(), req)
	if err != nil {
	...
	// OpenAI คืนมาเป็น []float64 ต้องแปลงเป็น []float32 สำหรับ Pinecone
